...
exports.run = async (client, message, args) => {
  ...
  let Sentius  = message.mentions.users.first();
  if (!args[0])
    return message.channel.send(
  ...
  if (
    message.guild.members.get(Sentius .id).highestRole.calculatedPosition >
  ...
  if (!message.guild.members.get(Sentius .id).voiceChannel)
    return message.channel.send(
    ...
    .then(() =>
      message.channel.send(
        mb
          ...
          .addField(`İşlem yapılan:`, Sentius .tag, true)
          .addField(
            `Süre:`,
            süre
              .replace(/d/, " gün")
